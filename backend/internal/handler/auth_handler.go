...
func (h *AuthHandler) ensureBackendModeAllowsUser(ctx context.Context, user *service.User) error {
	...
	return infraerrors.Forbidden("BACKEND_MODE_ADMIN_ONLY", "Backend mode is active. Only admin/operator login is allowed.")
}
...
func (h *AuthHandler) RefreshToken(c *gin.Context) {
	...
	// Backend mode: block users without backend access.
	if h.settingSvc.IsBackendModeEnabled(c.Request.Context()) && result.UserRole != "admin" && result.UserRole != "operator" {
		response.Forbidden(c, "Backend mode is active. Only admin/operator login is allowed.")
		return
	}
